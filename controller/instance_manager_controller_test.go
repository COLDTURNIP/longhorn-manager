...
func newDataEngineIPFamilySetting(value string) *longhorn.Setting {
	return newSetting(string(types.SettingNameDataEngineIPFamily), value)
}
...
func createDangerZoneSettingsForV2(c *C, lhClient *lhfake.Clientset, sIndexer cache.Indexer) {
	...
	overrides := map[types.SettingName]string{
		...
		types.SettingNameDataEngineIPFamily:        types.DataEngineIPFamilyIPv4,
	}
...
func (s *TestSuite) TestIsDataEngineIPFamilySynced(c *C) {
	testCases := []struct {
		...
		},
		{
			...
			attached:      true,
			expectedError: "failed to apply data-engine-ip-family setting to Longhorn components when there are attached volumes. It will be eventually applied",
		},
		{
			name: "blank setting matches omitted flag",
			containers: []corev1.Container{{
				Name: "instance-manager",
				Args: []string{"daemon"},
			}},
		...
		{
			name:         "blank setting accepts legacy explicit IPv4",
			settingValue: "",
			containers: []corev1.Container{{
				Name: "instance-manager",
				Args: []string{"--ip-family", types.DataEngineIPFamilyIPv4},
...
func (s *TestSuite) TestCreateV2InstanceManagerPodSpecUsesDualStackListener(c *C) {
	...
	for _, dataEngine := range []longhorn.DataEngineType{longhorn.DataEngineTypeV1, longhorn.DataEngineTypeV2} {
		...
		c.Assert(specified, Equals, true, Commentf("data engine: %s", dataEngine))
		c.Assert(family, Equals, types.DataEngineIPFamilyIPv4, Commentf("data engine: %s", dataEngine))
	...
	familySetting, err := lhClient.LonghornV1beta2().Settings(TestNamespace).Get(
		context.TODO(), string(types.SettingNameDataEngineIPFamily), metav1.GetOptions{})
	c.Assert(err, IsNil)
	familySetting.Value = ""
...
func (s *TestSuite) TestDataEngineIPFamilyStorageNetworkCompatibility(c *C) {
	...
	expectedMessage := fmt.Sprintf("Settings [data-engine-ip-family] are not synced: storage network longhorn-system/ipv4-only cannot provide an address in family ipv6 for pod %s/instance-manager-aio-v1-node1", TestNamespace)
	...
	c.Assert(condition.Status, Equals, longhorn.ConditionStatusFalse)
	c.Assert(condition.Reason, Equals, longhorn.InstanceManagerConditionReasonSettingNotSynced)
	c.Assert(condition.Message, Equals, expectedMessage)

	pod.Spec.Containers[0].Args = []string{"--ip-family", types.DataEngineIPFamilyIPv4}
	...
	recoveryFamilySetting, err := recoveryLHClient.LonghornV1beta2().Settings(TestNamespace).Get(context.TODO(), string(types.SettingNameDataEngineIPFamily), metav1.GetOptions{})
	c.Assert(err, IsNil)
