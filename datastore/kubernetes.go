...
func isDataEngineIPFamily(ip, family string) bool {
	return commonnet.IsUsableIPForFamily(net.ParseIP(ip), commonnet.IPFamily(family))
}

func getPrimaryDataEngineIP(pod *corev1.Pod) string {
	if pod == nil {
		return ""
	}
...
func (s *DataStore) GetDataEngineIPFromPodForContainer(pod *corev1.Pod, containerName string) (string, error) {
	...
	if !specified {
		return getPrimaryDataEngineIP(pod), nil
	}
...
func (s *DataStore) GetDataEngineIPFromPodByCNISettingForContainer(pod *corev1.Pod, settingName types.SettingName, containerName string) (string, error) {
	...
	if !specified {
		return s.getDataEngineIPFromCNISettingWithFallback(pod, settingName)
	}
...
func (s *DataStore) getDataEngineIPFromCNISettingWithFallback(pod *corev1.Pod, settingName types.SettingName) (string, error) {
...
func (s *DataStore) GetDataEngineIPFromPod(pod *corev1.Pod) string {
	...
	if !found || !specified || !valid {
		ip, err := s.getDataEngineIPFromCNISettingWithFallback(pod, types.SettingNameStorageNetwork)
		if err != nil {
			return pod.Status.PodIP
		}
		return ip
