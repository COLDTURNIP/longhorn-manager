...
func TestDataEngineIPFamilySetting(t *testing.T) {
	const expectedName = SettingName("data-engine-ip-family")

	if SettingNameDataEngineIPFamily != expectedName {
		t.Fatalf("unexpected setting name: got %q, want %q", SettingNameDataEngineIPFamily, expectedName)
	}

	definition, ok := GetSettingDefinition(SettingNameDataEngineIPFamily)
	if !ok {
		t.Fatalf("setting definition %q does not exist", SettingNameDataEngineIPFamily)
	}
	...
	if definition.Default != "" {
		t.Errorf("unexpected setting default: got %q, want empty", definition.Default)
	}
	if definition.Required {
		t.Error("data-engine IP family setting must not be required")
	}
	if definition.ReadOnly {
		t.Error("data-engine IP family setting must not be read-only")
	}
	if definition.DataEngineSpecific {
		t.Error("data-engine IP family setting must not be data-engine specific")
	}

	expectedChoices := []any{"", DataEngineIPFamilyIPv4, DataEngineIPFamilyIPv6}
	...
	for _, name := range SettingNameList {
		if name == SettingNameDataEngineIPFamily {
			foundInSettingNameList = true
			break
		}
	}
	if !foundInSettingNameList {
		t.Errorf("setting %q is missing from SettingNameList", SettingNameDataEngineIPFamily)
	}

	for _, value := range []string{"", DataEngineIPFamilyIPv4, DataEngineIPFamilyIPv6} {
		if err := ValidateSetting(string(SettingNameDataEngineIPFamily), value); err != nil {
